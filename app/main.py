...
from utils.dist_raw import dist_raw
from konlpy.tag import Okt
import time
import pandas as pd
from kss import split_sentences
from utils.cleaner import cleaner
from utils.honorific_token_counter import honorific_token_counter
...
# def run(target):
#     analysis(target, cleaner(df['비즈니스']))
#     analysis(target, cleaner(df['썸']))
#     analysis(target, cleaner(df['친구']))
#     analysis(target, cleaner(df['연인']))
def run(target):
    isBusiness, value = honorific_analysis(target)
    if isBusiness: print(value)
    else: analysis(target, [' '.join(cleaner(df['비즈니스']))])
    print()
    print()
    analysis(target, [' '.join(cleaner(df['썸']))])
    analysis(target, [' '.join(cleaner(df['친구']))])
    analysis(target, [' '.join(cleaner(df['연인']))])
